...
Normally only `TZ` (and optionally `JAVA_TOOL_OPTIONS`) need changing. Leave `NEXTCP_DATA` and `NEXTCP_LIB` at their defaults unless you deliberately relocate those directories.
