...
The following example uses `docker compose` to set up nextcp/2 along with Universal Media Server (UMS) in a single command. While UMS 16 or newer is the recommended media server, you can also use any other DLNA-compatible media server, though with fewer features.

The `ik6666/ums` image is built from the [UMS fork](https://github.com/ik666/UniversalMediaServer), where the features nextCP/2 relies on are developed before they are merged into UMS. It is therefore the build that matches the current nextCP/2 release most closely — an official UMS 16 image works as well, it may just lack the newest fixes.
...
| `NEXTCP_PORT` | `8085` | HTTP listen port used when the **default config is first generated**. Handy to avoid a port clash. Once a config exists, change the port there (`embeddedServerPort`) instead — this variable only seeds the initial value. |
| `NEXTCP_BIND_INTERFACE` | *(auto-detected)* | Network interface for UPnP discovery and the stream server (`upnpBindInterface`, e.g. `eth0`). If unset, the container auto-detects the Docker host's primary interface on first start. Set it explicitly only if the wrong interface is picked (e.g. multiple NICs). |
...
nextCP/2 **auto-detects the Docker host's primary network interface** on first start and sets `Bind Interface Stream Server` accordingly — usually no action is needed here. Verify it matches the interface you selected in the UMS configuration.

Only if the wrong interface was picked (e.g. the host has several NICs), correct it: set the `NEXTCP_BIND_INTERFACE` environment variable to the desired interface name and restart the container, or enter it in the `Bind Interface Stream Server` field and save.
