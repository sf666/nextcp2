 ...
 */
public class QPlayServiceSubscription extends RemoteGENASubscription
{
    private static final Logger log = LoggerFactory.getLogger(QPlayServiceSubscription.class.getName());

    private List<IQPlayServiceEventListener> eventListener = new CopyOnWriteArrayList<>();
        
    protected QPlayServiceSubscription(RemoteService service, int requestedDurationSeconds)
    {
    ...
    private void fakeStateChange(Long value)
    {
        for (IQPlayServiceEventListener listener : eventListener)
        {
            listener.fakeStateChange(value);
        }
    }    
}
