 ...
 */
public class webos-second-screenServiceSubscription extends RemoteGENASubscription
{
    private static final Logger log = LoggerFactory.getLogger(webos-second-screenServiceSubscription.class.getName());

    private List<Iwebos-second-screenServiceEventListener> eventListener = new CopyOnWriteArrayList<>();
        
    protected webos-second-screenServiceSubscription(RemoteService service, int requestedDurationSeconds)
    {
    ...
    private void x_PlaceHolderChange(String value)
    {
        for (Iwebos-second-screenServiceEventListener listener : eventListener)
        {
            listener.x_PlaceHolderChange(value);
        }
    }    
}
