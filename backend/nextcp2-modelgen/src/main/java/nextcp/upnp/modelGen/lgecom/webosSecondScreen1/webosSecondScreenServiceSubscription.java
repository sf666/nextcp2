 ...
 */
public class webosSecondScreenServiceSubscription extends RemoteGENASubscription
{
    private static final Logger log = LoggerFactory.getLogger(webosSecondScreenServiceSubscription.class.getName());

    private List<IwebosSecondScreenServiceEventListener> eventListener = new CopyOnWriteArrayList<>();
        
    protected webosSecondScreenServiceSubscription(RemoteService service, int requestedDurationSeconds)
    {
    ...
    {
        for (IwebosSecondScreenServiceEventListener listener : eventListener)
        {
