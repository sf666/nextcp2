 ...
 */
public class Log2ServiceSubscription extends RemoteGENASubscription
{
    private static final Logger log = LoggerFactory.getLogger(Log2ServiceSubscription.class.getName());

    private List<ILog2ServiceEventListener> eventListener = new CopyOnWriteArrayList<>();
        
    protected Log2ServiceSubscription(RemoteService service, int requestedDurationSeconds)
    {
    ...
    public void eventReceived()
    {
        ...
        for (StateVariableValue<RemoteService> stateVar : values.values())
        {
            ...
            try
            {
                ...
                {
                    case "LogStart":
                        logStartChange(((UnsignedVariableInteger) stateVar.getValue()).getValue());
                        break;
                    case "LogDataLength":
                        logDataLengthChange(((UnsignedVariableInteger) stateVar.getValue()).getValue());
                        break;
                    case "LogLevel":
                        logLevelChange((String) stateVar.getValue());
                        break;
                    default:
    ...
    private void logStartChange(Long value)
    {
        for (ILog2ServiceEventListener listener : eventListener)
        {
            listener.logStartChange(value);
        }
    }    
    ...
    private void logLevelChange(String value)
    {
        for (ILog2ServiceEventListener listener : eventListener)
        {
            listener.logLevelChange(value);
        }
    }    
}
