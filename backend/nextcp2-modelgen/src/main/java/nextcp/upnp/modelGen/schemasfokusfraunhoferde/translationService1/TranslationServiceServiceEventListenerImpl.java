...
public class TranslationServiceServiceEventListenerImpl implements ITranslationServiceServiceEventListener 
{
	...
	public TranslationServiceServiceEventListenerImpl(RemoteDevice device) {
		this.device = device;
	}
     ...
     */
    public TranslationServiceServiceStateVariable getStateVariable()
    {
    ...
    {
        stateVariable.LanguageIDList = value;
        if (log.isDebugEnabled())
        {
            log.debug(String.format("StateVariable : %s: %s", "LanguageIDList", value));
        }
    }
