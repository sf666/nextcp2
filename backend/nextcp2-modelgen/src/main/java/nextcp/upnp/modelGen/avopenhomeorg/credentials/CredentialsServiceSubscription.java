 ...
 */
public class CredentialsServiceSubscription extends RemoteGENASubscription
{
    private static final Logger log = LoggerFactory.getLogger(CredentialsServiceSubscription.class.getName());

    private List<ICredentialsServiceEventListener> eventListener = new CopyOnWriteArrayList<>();
        
    protected CredentialsServiceSubscription(RemoteService service, int requestedDurationSeconds)
    {
    ...
    private void sequenceNumberChange(Long value)
    {
        for (ICredentialsServiceEventListener listener : eventListener)
        {
