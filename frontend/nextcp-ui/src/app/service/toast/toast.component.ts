import { Component, ChangeDetectionStrategy, signal, AnimationCallbackEvent, inject } from '@angular/core';
...
export class ToastComponent {
  readonly data = inject(ToastData);
  readonly ref = inject(ToastRef);
  toastConfig = inject<ToastConfig>(TOAST_CONFIG_TOKEN);
  ...
  getColor(): string {
    ...
    switch (this.data.type) {
      case 'success':
        return 'inline-flex items-center justify-center shrink-0 w-8 h-8 rounded-lg bg-green-800 text-green-200';
      case 'warning':
        return 'inline-flex items-center justify-center shrink-0 w-8 h-8 rounded-lg bg-red-800 text-red-200';
      case 'info':
        return 'inline-flex items-center justify-center shrink-0 w-8 h-8 rounded-lg bg-blue-800 text-blue-200';
      default:
        return 'inline-flex items-center justify-center shrink-0 w-8 h-8 rounded-lg bg-blue-800 text-blue-200';
    }
  }
