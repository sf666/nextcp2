...
export class FooterComponent implements OnInit {
  ...
  sliderPos = computed(() =>
    this.reverseLogVal(this.rendererService.deviceDriverState().volume),
  );
  ...
  ngOnInit(): void {
    this.volControl.valueChanges
      ...
      .subscribe((val) => {
        ...
        const norm = val / 100;
        const logVal = Math.round(
          this.minVal +
            (this.maxVal - this.minVal) * Math.pow(norm, this.expVal),
        );
  ...
  showSongPopup(event: MouseEvent): void {
    if (this.rendererService.trackInfo().currentTrack?.streamingURL) {
      this.songOptionsServiceService.openOptionsDialog(
        event,
        this.rendererService.trackInfo().currentTrack,
        undefined,
      );
    }
  }
  ...
  currentInputSource(): string {
    if (this.deviceService.selectedMediaRendererDevice().currentSource?.Name) {
      return this.deviceService.selectedMediaRendererDevice().currentSource
        .Name;
    }
    return '';
  }
